...
2026년 6월 18일 남자아이돌 댄서 모집 프로젝트의 운영·집계는 `ndol26` 부모 프로젝트 기준으로도 볼 수 있게 한다.
...
공개 지원 흐름은 기존 분산 프로젝트를 우선한다.

따라서 `/projects/ndol02`, `/projects/ndolsm`, `/projects/ndolbd`, `/projects/ndol37` 같은 기존 링크는 각자의 프로젝트 상세로 열린다.

채널 링크(`/c/{share_code}`)도 `legacy_project_id`가 있으면 해당 레거시 프로젝트의 `?channel={share_code}` URL로 이동한다.

신규 지원이 발생하면 지원서의 `project_id`는 실제로 열린 레거시 프로젝트를 유지하고, `recruitment_channel_id`는 부가 메타데이터로만 붙인다.
...
- 공개 레거시 링크 처리: `/projects/{legacyShortCode}`는 해당 레거시 프로젝트 상세를 그대로 표시
- 채널 링크 처리: `/c/{share_code}`는 `legacy_project_id`가 있으면 `/projects/{legacyShortCode}?channel={share_code}`로 이동
...
5. 기존 공개 모집 링크는 각 레거시 프로젝트 상세로 열려야 한다.
6. `/c/{share_code}`는 `legacy_project_id`가 있으면 해당 레거시 프로젝트의 채널 URL로 열려야 한다.
7. 2026년 6월 18일 23:59 KST 이후 신규 지원은 마감 처리되어야 한다.

## 후속 주의사항

레거시 일정 응답 데이터는 기존 프로젝트에 그대로 남긴다.

새 일정 요청, 현장 운영, 참가자 시딩은 부모 프로젝트 기준으로 진행할 수 있되, 연결 채널 지원자까지 대상 계산에 포함한다.

외부 지원 링크와 지원서 저장 기준은 기존 분산 프로젝트 우선을 기본값으로 둔다.
