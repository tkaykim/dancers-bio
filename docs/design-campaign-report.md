...
페이 산정과 같이 `(app)` 레이아웃 아래에 두며 `/admin` 레이아웃에 넣지 않는다.
...
- 표 열은 계정·참여자·게시물 링크 및 shortcode·게시일·팔로워·재생·좋아요·댓글·공유·준수·게시물 상태·메모다.
...
- 회차 표에 라벨·측정 시각·상태·대상 및 확인 수·재생·좋아요·댓글·공유 커버리지·실행자를 표시한다.
