...
### 5.5 관리자 취소 및 일괄 처리 (2026-08-11 구현)
- 관리자는 정산 상세에서 테스트·중복·지급 대상이 아닌 `pending` 또는 `requested` 건을 2단계 확인 후 `cancelled`로 전환할 수 있다.
- `paid` 건은 장부 정합성을 위해 취소할 수 없고, 상태 변경 시점이 겹치면 조건부 업데이트가 실패하도록 보호한다.
- 취소된 건은 관리자 대기열, 댄서의 `/me/settlements`, 공유 출금 화면 `/w/[code]`에서 제외된다.
- 상단 `전체 선택 (N명)`은 금액과 계좌가 모두 준비된 미지급 건만 선택하며, 다계좌이체 파일 생성과 일괄 입금완료가 같은 선택 집합을 사용한다.
