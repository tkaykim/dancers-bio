...
export function ForecastSummary({ forecast }: { forecast: ForecastSummaryData }) {
  const { counts, all, confirmed, tiers, settings } = forecast;
    ...
    <section className="mt-5">
      <div className="grid gap-3 sm:grid-cols-2 lg:grid-cols-4">
        <Stat
          ...
          sub={`확정 ${counts.confirmed} · 협의 중 ${counts.negotiating} · 제안 예정 ${counts.proposed}`}
        />
        <Stat
          label="팔로워 합계"
          value={formatKoCount(all.followers)}
          sub={`확정 인원 ${formatKoCount(confirmed.followers)}`}
        />
        <Stat
          label={`예상 조회수 · ${settings.horizonLabel}`}
          value={`${formatKoRange(all.views.low, all.views.high)} 회`}
          sub={`기준 ${formatKoCount(all.views.base)} 회 · 확정 인원 ${formatKoRange(confirmed.views.low, confirmed.views.high)} 회`}
          accent
        />
        <Stat
          label="예상 상호작용"
