...
export function DeliveryReport({ report }: { report: PublicReport }) {
  ...
  return <article className={styles.report}>
    ...
    <header className={styles.header}>
      ...
      <div className={styles.timestamp}>조회수 기준 {stamp(report.snapshot.takenAt)} · 한국 시간</div>
      {d.followersCheckedAt && <div className={styles.followerTime}>팔로워 확인 {stamp(d.followersCheckedAt)} · 한국 시간</div>}
    </header>
    ...
    <section className={styles.section}>
      <div className={styles.sectionHead}><div><h2>업로드 완료 <span>{d.posts}</span></h2></div>
        <label className={styles.search}><Search size={16}/><input aria-label="참여자 검색" placeholder="참여자 검색" value={search} onChange={e => setSearch(e.target.value)}/></label>
      </div>
