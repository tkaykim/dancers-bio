...
/**
 * SNS 아이콘 버튼 줄. 히어로(어두운 배경) 위에 흰색 아이콘으로 표시.
 * social_links 의 instagram/youtube/tiktok 키만 렌더 (값은 핸들·URL 모두 허용).
 */
export function SocialIconRow({
  ...
  const links = social ?? {};
  const items = PLATFORMS.filter((p) => links[p]?.trim());
  ...
  return (
    <div className={"flex items-center gap-2 " + className}>
      {items.map((p) => (
        <Link
          key={p}
          href={normalizeSocialUrl(p, links[p])}
          target="_blank"
          rel="noopener"
          aria-label={LABELS[p]}
