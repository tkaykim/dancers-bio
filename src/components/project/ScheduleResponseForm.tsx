...
import { submitScheduleResponseAction } from "@/app/actions/project-schedules";

type Status = "available" | "partial" | "unavailable";
type Slot = { start: string; end: string; kind: "available" | "unavailable" };

export function ScheduleResponseForm({
  token,
  current,
  currentSlots,
}: {
  ...
  const OPTIONS: { v: Status; label: string; desc: string }[] = [
    { v: "available", label: "가능", desc: "전체 시간 참석 가능" },
    { v: "partial", label: "시간 일부", desc: "특정 시간만 가능/불가" },
  ...
  function submit() {
    ...
    if (status === "partial") {
      const clean = slots.filter((s) => s.start && s.end);
      fd.set("time_slots", JSON.stringify(clean));
    }
    if (note.trim()) fd.set("note", note.trim());
    startTransition(async () => {
      const r = await submitScheduleResponseAction(fd);
      if (!r.ok) {
        setError(r.error);
        return;
      }
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <textarea
        ...
        rows={2}
        placeholder="전달할 메모가 있으면 적어주세요 (선택)"
