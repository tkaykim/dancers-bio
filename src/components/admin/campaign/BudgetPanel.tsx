...
export function BudgetPanel({data}:{data:BudgetData}) {
  ...
  return <section className="space-y-5" aria-label="캠페인 예산">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-lg font-bold">예산 소요 현황</h2><p className="text-xs text-ink-3">대표 전용 · 클라이언트 보드와 참여자에게 공개되지 않습니다.</p></div>
      <Link className={buttonClass} href={`/projects/${project}/settlements`}>정산·지급 현황 ↗</Link></div>
    <div className="grid grid-cols-2 gap-3 lg:grid-cols-3">{cards.map(([label,value,detail])=><div key={label} className="rounded-2xl border border-border p-4"><p className="text-xs text-ink-3">{label}</p><p className="my-2 break-words text-xl font-bold tabular-nums">{value}</p><p className="text-xs text-ink-3">{detail}</p></div>)}</div>
    {(!summary.complete || summary.outsideCount>0) && <p className="rounded-xl bg-secondary p-4 text-sm">{summary.unpriced}명의 출연료와 {summary.operations===null?"운영비·예비비가 미확인입니다.":"운영비 입력 기준을 확인해 주세요."}
    ...
    <p className="text-xs leading-relaxed text-ink-3">필요 예상액은 참여자별 입력 금액과 출연료 정산 등록액 중 큰 값에 기타 정산·운영비 확보액을 더합니다.<br/>희망 단가는 참고값이며 예상액에 자동 합산하지 않습니다.<br/>같은 출연료를 예산과 정산에 입력해도 중복 합산하지 않으며, 모든 입력은 원화 기준입니다.</p>
    <Editor title="예산·운영비 설정"><form key={data.settings.version} className="space-y-4" onSubmit={e=>{e.preventDefault();const form=new FormData(e.currentTarget);setMessage("");action.run(()=>saveBudgetAction(project,"configure",{version:data.settings.version,total_amount:amount(form.get("total")),operations_reserve:amount(form.get("operations")),basis:form.get("basis")}),()=>setMessage("예산 설정을 저장했습니다."));}}>
    ...
    <div className="overflow-x-auto"><table className={tableClass}><thead><tr>{["참여자","희망 단가 · 참고","입력 금액","상태","정산 등록","필요 예상액","관리"].map(h=><th key={h}>{h}</th>)}</tr></thead><tbody>{summary.rows.filter(r=>r.participant.display_name.toLowerCase().includes(search.toLowerCase())&&(!missingOnly||r.required===null)).map(row=><tr key={row.participant.id}>
      <td>{row.participant.display_name}</td><td>{row.quote?.proposed_fee!=null?`${number(row.quote.proposed_fee)} ${row.quote.proposed_fee_currency??"통화 미확인"} / ${row.quote.proposed_fee_unit??"단위 미확인"}`:"미등록"}</td><td>{won(row.fee?.amount)}</td><td>{row.fee?.amount==null?"미확인":row.fee.status==="agreed"?"합의 완료":"협의 예상"}</td><td>{won(row.registered)}</td><td>{won(row.required)}</td><td>{data.settings.version ? <FeeEditor project={project} row={row}/> : "예산 설정 후 입력"}</td>
    </tr>)}</tbody></table></div>
  </section>;
}
