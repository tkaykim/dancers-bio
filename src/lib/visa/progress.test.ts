...
test("신청·미팅은 본 프로그램 진행률에 포함하지 않는다", () => {
  assert.equal(deriveVisaProgress({ ...base, caseStage: "triage_submitted" }).activeStep, 1);
});
...
test("공식 발급이 확인된 경우에만 마지막 단계로 이동한다", () => {
  assert.equal(deriveVisaProgress({ ...base, visaIssuedAt: "2026-08-24T00:00:00.000Z" }).activeStep, 5);
});
