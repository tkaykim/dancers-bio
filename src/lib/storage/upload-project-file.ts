...
export type UploadedProjectFile = ProjectAttachmentDraft;
...
export async function uploadProjectFileFromBrowser(
  ...
  if (!isAllowedProjectFileMime(file.type))
    return {
      ok: false,
      error: "PDF·이미지(JPG/PNG/WebP/GIF)·영상(MP4/WebM/MOV)만 업로드할 수 있습니다.",
    };
  ...
  const path = `${user.id}/${Date.now()}-${rand}-${safe}`;
...
export async function deleteUploadedProjectFileFromBrowser(
  path: string,
  ...
  const { error } = await supabase.storage
    .from(PROJECT_FILES_BUCKET)
    .remove([path]);
  if (error) return { ok: false, error: `파일 정리 실패: ${error.message}` };
  return { ok: true };
}
