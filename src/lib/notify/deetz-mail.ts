...
import { mailT } from "@/lib/i18n/mail-messages";
...
export function renderDeetzMail(params: {
  ...
  footerLines?: string[];
  /** 껍데기(태그라인·발송 안내·lang)의 언어. 기존 호출부는 그대로 한국어. */
  locale?: Locale;
}): string {
  const {
    pill,
  ...
  const tagline = escapeHtml(mailT(locale, "mail.brand.tagline"));
  const sentNotice = escapeHtml(mailT(locale, "mail.brand.sent_notice"));
  ...
  return `<html lang="${locale}"><body style="margin:0;padding:0;background:#f4f4f5;">
...
<tr><td style="padding:28px 32px 18px;border-bottom:1px solid #ececef;">
  ...
  <div style="font-size:12px;color:#6b7280;margin-top:10px;">${tagline}</div></td></tr>
...
<tr><td style="padding:22px 32px 28px;border-top:1px solid #ececef;background:#fafafa;">
  ...
  <div style="font-size:12px;color:#6b7280;margin:10px 0 14px;">${tagline}</div>
  ...
  <div style="font-size:11px;color:#a1a1aa;margin-top:12px;line-height:1.6;">© 2026 deetz. All rights reserved.<br>${sentNotice}</div></td></tr>
...
export async function resolveApplicantContact(
  ...
  const fallbackName = mailT(locale, "mail.recipient.fallback_name");
  ...
  if (name === fallbackName && applicantId) {
