...
export function buildScheduleRequestEmail(params: {
  ...
  whenText: string; // 예: "6월 18일(수) 16:00~21:00"
  locationText?: string | null;
  url: string;
}): { subject: string; text: string; html: string } {
  const { name, scheduleLabel, whenText, locationText, url } = params;
  const subject = `[deetz] ${name}님, 일정 참석 가능 여부를 알려주세요`;

  const text = [
    `${name}님, 안녕하세요.`,
    ``,
    `참여하신 프로젝트의 일정이 잡혀 안내드립니다.`,
    `일정: ${scheduleLabel}`,
    `일시: ${whenText}`,
    ...(locationText ? [`장소: ${locationText}`] : []),
    ``,
    `아래 링크에서 로그인 없이 참석 가능 여부만 눌러 주세요. (가능 / 시간 일부 / 불가)`,
    `${url}`,
    ``,
...
<p style="font-size:15px;line-height:1.75;color:#33363b;margin:0;">참여하신 프로젝트의 일정이 잡혀 안내드립니다.<br>아래에서 <b>참석 가능 여부</b>만 눌러 주시면 됩니다.</p></td></tr>
<tr><td style="padding:18px 32px 6px;"><div style="background:#f6f6f7;border:1px solid #ececef;border-radius:14px;padding:16px 18px;">
<table role="presentation" cellpadding="0" cellspacing="0" width="100%"><tbody>
<tr><td style="vertical-align:top;width:56px;color:#6b7280;font-size:13px;padding:4px 0;">일정</td><td style="font-size:14px;font-weight:700;color:#111;padding:4px 0;">${esc(scheduleLabel)}</td></tr>
<tr><td style="vertical-align:top;width:56px;color:#6b7280;font-size:13px;padding:4px 0;">일시</td><td style="font-size:14px;color:#33363b;padding:4px 0;">${esc(whenText)}</td></tr>
${locationText ? `<tr><td style="vertical-align:top;width:56px;color:#6b7280;font-size:13px;padding:4px 0;">장소</td><td style="font-size:14px;color:#33363b;padding:4px 0;">${esc(locationText)}</td></tr>` : ""}
</tbody></table></div></td></tr>
<tr><td style="padding:16px 32px 24px;"><a href="${url}" style="display:block;background:#111;color:#fff;text-decoration:none;text-align:center;font-size:15px;font-weight:700;padding:15px 0;border-radius:12px;">참석 가능 여부 알려주기 →</a>
