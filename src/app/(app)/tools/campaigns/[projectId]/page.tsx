...
export default async function CampaignPage({
  ...
  const [data, boards, submissions] = await Promise.all([
    loadCampaign(projectId),
    boardOptions(projectId),
    loadSubmissions(projectId),
  ]);
  const tab = ["submissions", "posts", "trend", "reports"].includes(
    String(query.tab),
  )
    ? String(query.tab)
    : submissions.settings.enabled
      ? "submissions"
      : "posts";
  ...
  return (
    <main className="mx-auto flex max-w-7xl flex-col gap-6 px-6 py-8">
        ...
        <>
          <div className="grid items-start gap-4 md:grid-cols-2">
            ...
            <SnapshotDialog
              projectId={projectId}
              postsTotal={
                data.posts.filter((p) => p.status !== "excluded").length
              }
      ...
      {tab === "submissions" && (
        <SubmissionsPanel initial={submissions} boards={boards} />
      )}
      {tab === "posts" && (
        <PostsTable
          posts={data.posts}
