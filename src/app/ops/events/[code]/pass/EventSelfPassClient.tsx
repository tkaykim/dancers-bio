...
export function EventSelfPassClient({
  opsCode,
  eventLabel,
}: {
  opsCode: string;
  ...
  const selfPassPath = `/ops/events/${opsCode}/pass`;
  const loginHref = `/login?redirect=${encodeURIComponent(selfPassPath)}`;
  const apiPath = `/api/ops/events/${opsCode}/self-pass`;
  ...
  useEffect(() => {
    ...
    async function loadLinkedPass() {
      setAutoLoading(true);
      try {
        const response = await fetch(apiPath, { method: "GET", cache: "no-store" });
        ...
        <section>
          <div className="flex items-start justify-between gap-3 border-b border-black/10 pb-4">
            <div>
              ...
              <h1 className="mt-2 text-2xl font-black tracking-tight">현장 접수 QR</h1>
            </div>
            <div
              aria-label="dee'tz"
              ...
              <div className="flex items-start justify-between gap-3">
                <div>
                  ...
                  <div className="mt-1 text-sm font-bold text-black/45">
                    {pass.channelName ?? eventLabel} · {pass.genderLabel}
                  </div>
                </div>
                <button
                  type="button"
