...
begin
  ...
  account_number := regexp_replace(coalesce(payout.bank_account_number, ''), '[[:space:]-]', '', 'g');
  resident_number := regexp_replace(coalesce(payout.resident_registration_number, ''), '[[:space:]-]', '', 'g');
